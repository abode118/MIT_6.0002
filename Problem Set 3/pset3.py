...
class RectangularRoom(object):
    ...
    def clean_tile_at_position(self, pos, capacity):
        ...
        currenttile = (math.floor(pos.get_x()),math.floor(pos.get_y()))
        if (self.tiledict[currenttile] - capacity) >= 0:
            self.tiledict[currenttile] -= capacity
        else:
            self.tiledict[currenttile] = 0
    ...
    def is_position_in_room(self, pos):
        ...
        return (math.floor(pos.get_x()),math.floor(pos.get_y())) in \
        self.tiledict.keys()
...
class EmptyRoom(RectangularRoom):
    ...
    def is_position_valid(self, pos):
        ...
        return newtile in self.tiledict
        
    def get_random_position(self):
        """
        Returns: a Position object; a valid random position (inside the room).
        """
        random_position = Position(random.uniform(0,self.width-1), 
...
class FurnishedRoom(RectangularRoom):
    ...
    def is_position_furnished(self, pos):
        ...
        return newtile in self.furniture_tiles
    ...
    def is_position_valid(self, pos):
        ...
        return self.is_position_furnished(pos) == False and \
        self. is_position_in_room(pos) == True
        
    def get_num_tiles(self):
        """
        Returns: an integer; the total number of tiles in the room that can be
        accessed.
        """
    ...
    def get_random_position(self):
        ...
        random_position = Position(random.uniform(0,self.width-1),
                                   random.uniform(0,self.height-1))
        while self.is_position_furnished(random_position) == True:
            random_position = Position(random.uniform(0,self.width-1),
                                       random.uniform(0,self.height-1))
        return random_position
...
class StandardRobot(Robot):
    ...
    def update_position_and_clean(self):
        ...
        newposition = (currentposition.get_new_position(self.get_robot_direction(),
                                                        self.speed))

        if self.room.is_position_valid(newposition) == True:
            self.set_robot_position(newposition)
            self.room.clean_tile_at_position(newposition, self.capacity)
        else:
            newdirection = random.random() * 360
            self.set_robot_direction(newdirection)
...
def run_simulation(num_robots, speed, capacity, width, height, dirt_amount,
    ...
    for trial in range(num_trials):
        CurrentRoom = EmptyRoom(width, height, dirt_amount)
        robotlist = []
        for rob in range(num_robots):
            robotlist.append(robot_type(CurrentRoom,speed,capacity))
...
# anim = ps3_visualize.RobotVisualization(num_robots, width, height, False, 0.1)
        ...
        while cleaned < min_coverage:
            for TestRobots in robotlist:
                TestRobots.update_position_and_clean()
            clockticks += 1
# anim.update(CurrentRoom,robotlist)
            cleaned = CurrentRoom.get_num_cleaned_tiles()/CurrentRoom.get_num_tiles()

        trialresults.append(clockticks)
# anim.done()
        
    return(sum(trialresults)/len(trialresults))    
...
'''Standard robot is ~15% faster than FaultyRobot'''
