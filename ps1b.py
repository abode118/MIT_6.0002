...
def dp_make_weight(egg_weights, target_weight, memo = {}):
    ...
    for weight in egg_weights:
        toConsider.append(weight)
    
    def make_weight(toConsider, target_weight):
        if toConsider == [] or target_weight == 0:
            return 0
        
        elif target_weight - toConsider[-1] < 0:
            return make_weight(toConsider[:-1], target_weight)
    
        else:
            weightused = toConsider[-1]
            target_weight -= weightused
            return 1 + make_weight(toConsider, target_weight)

    return make_weight(toConsider, target_weight)
